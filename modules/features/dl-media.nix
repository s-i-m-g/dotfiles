{ self, inputs, ... }: {
  ...
  let
    dlMedia = pkgs.writeShellScriptBin "dl-media" ''
      ...
      # YouTube/Instagram/embeds go through yt-dlp; Discord attachment links
      # are direct CDN files and are fetched with curl.
      set -euo pipefail
      ...
      if [ "$site" = discord ]; then
        ...
        out="$(${pkgs.yt-dlp}/bin/yt-dlp \
          ...
          "$url" 2>/dev/null | tail -1)" || die "yt-dlp failed"
